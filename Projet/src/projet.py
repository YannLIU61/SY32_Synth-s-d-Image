# -*- coding: utf-8 -*-
"""
Created on Fri Mar 22 08:18:24 2019

@author: LI
"""
#boite englobante
import numpy as np
# ligne, colonne,hauteur, largeur
x_label = np.loadtxt("/Users/yann/Desktop/UTC/GI05/SY32_projet/data/label.txt")
ratioHL = np.mean(x_label[:,3]/x_label[:,4])
boxHauteurMoyenne = int(round(np.mean(x_label[:,3])))
boxLargeurMoyenne = int(round(np.mean((x_label[:,4]))))
...
def read_images_to_resize_gray_and_float_from_folder(folder):
    images = []
    files =os.listdir(folder)
    files.sort()
    for filename in files:
        img = io.imread(os.path.join(folder,filename))
        if img is not None:
            img = util.img_as_float(transform.resize(color.rgb2gray(img), [96,64])) # respest 1:1.5
            images.append(img)
    return images
...
def read_images_to_resize_gray_and_float_and_hog_from_folder(folder):
    ...
    for filename in files:
        img = io.imread(os.path.join(folder,filename))
        if img is not None:
            img = feature.hog(util.img_as_float(transform.resize(color.rgb2gray(img), [96,64])))
            images.append(img)
    return images

#charger de données
x_train = load_images_from_folder('/Users/yann/Desktop/UTC/GI05/SY32_projet/data/origin')
...
dataTrain = np.ravel(dataTrain).reshape(len(trainNeg) + len(trainPos),4860) # len(ravel(dataTrain))/(len(trainNeg) + len(trainPos))
...
#    resH = 48
    pas = 8
    for k in scale:
        longueur = np.shape(im)[1]
        hauteur = np.shape(im)[0]
        resL =int(64*k)
        resH =int(96*k)
        for y in range(0, hauteur-resH,pas):
            for x in range(0, longueur-resL, pas):
                if(x+resL < longueur and y+resH < hauteur):
                    carre = im[y:y+resH, x:x+resL]
                    carre = feature.hog(transform.resize(carre, [96,64]))
                    carrepredict = clf.predict(np.ravel(carre).reshape(1,4860))
                    score = clf.decision_function(np.ravel(carre).reshape(1,4860))
...
x_test = load_images_from_folder('/Users/yann/Desktop/UTC/GI05/SY32_projet/data/test') 

######################################test detecter Visage#####################################
    
result = fenetreglissante(util.img_as_float(color.rgb2gray(x_test[62])))
coord = result[0]
score = result[1]
...
for i in range(len(coord)):
    ...
    L = coord[i][2]
    H = coord[i][3]
    rect = patches.Rectangle((x,y),L,H,linewidth=1,edgecolor='r',facecolor='none')
    print(score[i][0])
    ax.add_patch(rect)
 
############generer detection.txt############
txt_path = '/Users/yann/Desktop/UTC/GI05/SY32_projet/data/detection.txt'
